...
public class JmmVisitor extends PreorderJmmVisitor<JmmSymbolTable, Void> {
    ...
    private Void dealWithMethod(JmmNode node, JmmSymbolTable parent) {
        ...
        /*
         * List<Symbol> parameters = new ArrayList<>(); List<Symbol> localVariables =
         * new ArrayList<>();
         * 
         * for (JmmNode child : node.getChildren()) { if
         * (child.getKind().equals("Arguments"))) { for (JmmNode child2:
         * child.getChildren()) { if (child2.getKind().equals("Argument")) { Type type =
         * typeFromString(child2.get("type")); Symbol variableSymbol = new Symbol(type,
         * child2.get("name")); parameters.add(variableSymbol); } } } else if
         * (child.getKind().equals("Body")) { for (JmmNode child2 : child.getChildren())
         * { if (child2.getKind().equals("VariableDeclaration")) { Type type =
         * typeFromString(child2.get("type")); Symbol variableSymbol = new Symbol(type,
         * child2.get("name")); localVariables.add(variableSymbol); }
         * 
         * } } }
         */
